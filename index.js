...
const memory_view = document.getElementById("memory");
const registers = document.getElementById("registers");
const stack_view = document.getElementById("stack");
const program = document.getElementById("program");
const current_instruction = document.getElementById("current_instruction");
const labels_view = document.getElementById("labels");
var label_addresses = {};
var g_wasm = null;
...
function assemble() {
    ...
    let asm_output = readRustString(outputPtr);
    ...
    if (asm_output.slice(0, 5) == "error") {
        ...
        reset();
        let code = asm_output.match(/.{1,2}/g);
        if (code != null) {
        ...
        asm_output = asm_output.replace(/\x1b\[1m/g, "");
        asm_output = asm_output.replace(/\x1b\[0m/g, "");
        let labels = Array.from(asm_output.matchAll(/ +([0-9a-f]+).+; (.+):/g));
        labels_view.innerHTML = "";
        label_addresses = {};
        if (labels != null) {
